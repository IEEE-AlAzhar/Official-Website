  ...
  componentDidMount() {
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting === true && window.innerWidth <= 767) {
        this.IEEEStrImg.current.innerHTML = `<img src=${IEEEStr650} class = "${styles.IEEE_str_img__structureImg}" alt="Tree shows the structure of the team" />`;
      } else if (entry.isIntersecting === true && window.innerWidth >= 1200) {
        this.IEEEStrImg.current.innerHTML = `<img src=${IEEEStr1200} class ="${styles.IEEE_str_img__structureImg}" alt="Tree shows the structure of the team"  />`;
      } else if (entry.isIntersecting === true && window.innerWidth >= 2500) {
        this.IEEEStrImg.current.innerHTML = `<img src=${IEEEStr2500} class = "${styles.IEEE_str_img__structureImg}" alt="Tree shows the structure of the team" />`;
      }
    });
